 ...
 */
async function harvestMatureOwnLandsOnce(actions, propagateErrors = false) {
    let latest;
    try {
 ...
 */
async function runFarmOperation(opType, targetLandIdInput = null, propagateErrors = false) {
    ...
    if (opType === 'all' || opType === 'clear') {
        ...
        if (!skipOwnWeedBug && farmingLandIds.length > 0) {
            try {
                ...
                if (hasTargetLandId || propagateErrors)
                    throw e;
            }
        }
    }
...
function onLandsChangedPush(lands) {
    ...
    farmScheduler.setTimeoutTask('farm_push_check', 100, async () => {
        if (!isCheckingFarm)
            await runExclusiveAutomationTask('farm_push_check', checkFarm);
    });
}
