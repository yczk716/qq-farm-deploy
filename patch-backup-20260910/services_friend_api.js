    ...
    if (isQQ) {
        ...
        try {
            ...
            if (getEffectiveKnownQqFriendGids().length === 0) {
                if (e?.name === 'GatewayError' || typeof e?.errorMessage === 'string')
                    throw e;
...
async function putPlantItems(friendGid, landIds, RequestType, ReplyType, method, propagateErrors = false) {
...
async function putPlantItemsDetailed(friendGid, landIds, RequestType, ReplyType, method, propagateErrors = false) {
