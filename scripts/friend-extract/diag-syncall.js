...
async function fetchAllGameFriends(sendMsgAsync, selfGid, seedGids) {
  ...
  try {
    ...
    try { require('fs').writeFileSync(process.env.DIAG_HEX_FILE || '/tmp/syncall-body.hex', replyBody.toString('hex')); } catch (_) {}
    const sd = decodeSyncAllReply(replyBody);
    debug.syncAllBodyLen = replyBody ? replyBody.length : 0;
    debug.mutualGids = sd.mutual.map((f) => f.gid).slice(0, 300);
    debug.recommendedGids = sd.recommended.map((f) => f.gid).slice(0, 300);
  ...
  if (candidateGids.length > 0) {
    try {
      ...
      debug.gfGids = all.map((f) => toNum(f && f.gid)).filter((g) => g > 0).slice(0, 500);
