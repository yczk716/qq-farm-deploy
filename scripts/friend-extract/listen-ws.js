...
(async () => {
  ...
  try {
    context = await connect(SCAN_CODE, async () => {
      ...
      const fr = await listenAll(sendMsgAsync, selfInfo.gid, seedGids);
      clearTimeout(totalTimer);
      if (checkTimer) clearInterval(checkTimer);
...
// ==================== 监听全部入站 ws 包（诊断用） ====================
// 思路：用户提示「全部好友在 ws 包里」。我们登录后 hook 底层 GateMessage.decode，
// 把每个入站包的 service/method/type/body 都 dump 出来，并尝试用自研解码器识别「含好友列表」的包。
// 主动请求 + 网关主动推送（notify）都会经过这里。
async function listenAll(sendMsgAsync, selfGid, seedGids) {
  const fs2 = require('fs');
  const CAP = '/tmp/ws-capture.jsonl';
  try { fs2.writeFileSync(CAP, ''); } catch (_) {}

  const dec = require('/opt/napcat-code-web/_decoder_only.js');

  // hook 所有入站 GateMessage
  const protoMod = require(path.join(FARM_DIST, 'utils', 'proto'));
  const types = protoMod.types;
  const GM = types.GateMessage;
  const origDecode = GM.decode.bind(GM);
  let captured = 0;
  GM.decode = function (buf) {
    const msg = origDecode(buf);
    try {
      const body = msg && msg.body ? Buffer.from(msg.body) : Buffer.alloc(0);
      const meta = msg && msg.meta ? msg.meta : {};
      // 尝试识别好友列表
      let friendCount = 0, source = '';
      try {
        const sa = dec.decodeSyncAllReply(body);
        if (sa.mutual.length || sa.recommended.length) {
          friendCount = sa.mutual.length + sa.recommended.length;
          source = 'SyncAllReply(mutual=' + sa.mutual.length + ',rec=' + sa.recommended.length + ')';
        }
      } catch (_) {}
      if (!friendCount) {
        try {
          const gf = dec.decodeGameFriends(body);
          if (gf.length) { friendCount = gf.length; source = 'GameFriendsReply'; }
        } catch (_) {}
      }
      if (!friendCount) {
        try {
          const recs = require(path.join(FARM_DIST, 'utils', 'proto'));
        } catch (_) {}
      }
      const entry = {
        n: ++captured,
        t: Date.now(),
        service: String(meta.service_name || ''),
        method: String(meta.method_name || ''),
        msgType: Number(meta.message_type),
        ...
        friendCount,
        source,
        bodyHex: body.length <= 4096 ? body.toString('hex') : body.slice(0, 4096).toString('hex') + '...',
      };
      try { fs2.appendFileSync(CAP, JSON.stringify(entry) + '\n'); } catch (_) {}
    } catch (_) {}
    return msg;
  };
  ...
  // 监听窗口（毫秒）：登录后等待网关主动下发 + 收集主动请求响应
  const LISTEN_MS = Math.max(5000, parseInt(process.env.LISTEN_MS || '10000', 10));

  // 主动触发几个可能返回好友的接口，并等待推送
  const probes = [
  ...
  for (const [svc, m, body] of probes) {
    try {
      const r = await sendMsgAsync(svc, m, body, { timeoutMs: 8000 });
      const b = r && r.body ? Buffer.from(r.body) : Buffer.alloc(0);
      try {
        const list = dec.decodeGameFriends(b);
        for (const f of list) add(f);
      } catch (_) {}
      try {
        const sa = dec.decodeSyncAllReply(b);
        for (const f of sa.mutual) add(f);
        for (const f of sa.recommended) add(f);
      } catch (_) {}
    } catch (e) {
      // 忽略，监听继续
    }
  ...
  // 等待网关主动推送
  await new Promise((res) => setTimeout(res, LISTEN_MS));

  // 从 dump 里二次分析：把每个含好友的包的 gid 都收集进来
  let extra = 0;
  try {
    const lines = fs2.readFileSync(CAP, 'utf8').split('\n').filter(Boolean);
    for (const ln of lines) {
      const e = JSON.parse(ln);
      if (!e.friendCount || !e.bodyHex || e.bodyHex.endsWith('...')) continue;
      const b = Buffer.from(e.bodyHex, 'hex');
      try {
        if (e.source && e.source.indexOf('SyncAllReply') === 0) {
          const sa = dec.decodeSyncAllReply(b);
          for (const f of sa.mutual.concat(sa.recommended)) add(f);
          extra++;
        } else if (e.source === 'GameFriendsReply') {
          const gf = dec.decodeGameFriends(b);
          for (const f of gf) add(f);
          extra++;
        }
      } catch (_) {}
    }
  ...
  return {
    ...
    debug: {
      ...
      captureFile: CAP,
      extraFromDump: extra,
      seeds: (Array.isArray(seedGids) ? seedGids.length : 0),
    },
  };
}
