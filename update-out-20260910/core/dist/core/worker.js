...
async function runUnifiedTick() {
    ...
    if (dueFarm)
        await runExclusiveAutomationTask('farm_tick', () => runFarmTick(auto));
    if (dueFriend)
        await runExclusiveAutomationTask('friend_tick', () => runFriendTick(auto));
}
...
function startMysteryShopTimer() {
    const { isMysteryShopWatchEnabled, AUTO_BUY_CHECK_INTERVAL_MS, } = require('../services/mystery-shop-auto');
...
async function getDailyGiftOverview(propagateErrors = false) {
    const auto = getAutomation() || {};
