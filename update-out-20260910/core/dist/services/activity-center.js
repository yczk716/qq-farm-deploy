...
const { createEmptyCharityRedFlowerState, loadCharityRedFlowerState, mergeCharityRedFlowerStates, mergeConstellationStates, persistCharityRedFlowerState, stateRecordKey, loadConstellationState, persistConstellationState, stateFromDynamicNodes, stateWithNoClaimableDay, } = require('./activity-center-state');
...
const { querySeason, querySolarTerms, normalizeSeason, normalizeSolarTerms, queryShopFromSeason, buildActions, constellationFromSeasonReply, getCurrentSeasonEvent, getCurrentStarSandShop, getCurrentSolarTerms, getCurrentStellarActivity, claimBattlePassRewards, exchangeStarSandGoods, lightConstellation, claimSolarTerm, } = createStellarActivityService({
    types,
    sendMsgAsync,
    GatewayError,
    getBag,
...
const { getCurrentCharityRedFlowerActivity, claimCharityRedFlowerSeeds, donateCharityRedFlowerLove, claimCharityRedFlowerDailyGift, claimCharityRedFlowerProgressReward, charityRedFlowerDto, reconcileCharityProgressState, } = createCharityActivityService({
    types,
    sendMsgAsync,
    GatewayError,
    getServerTimeSec,
...
const { getCurrentQixiActivity, claimQixiBridgeRewards, giftQixiSachet, } = createQixiActivityService({
    types,
    sendMsgAsync,
    getBag,
...
const { getCurrentQingMeiActivity, claimQingMeiDailySeed, startQingMeiBrew, continueQingMeiBrew, settleQingMeiBrew, } = createQingMeiActivityService({
    types,
    sendMsgAsync,
    GatewayError,
    getBag,
