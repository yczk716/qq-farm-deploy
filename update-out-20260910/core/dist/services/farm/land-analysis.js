...
const { getPlantName, getPlantExp, getPlantById, getSeedImageBySeedId, getPlantGrowTime, getPlantGrowPhases, getItemById, getMutantEffectsByIds, getMutantDisplayPlantId, } = require('../../config/gameConfig');
...
function buildLandDetail(land, options = {}) {
    ...
    const maturePhase = Array.isArray(plant.phases)
        ? plant.phases
            .filter((phase) => phase && toTimeSec(phase.begin_time) > 0)
...
function getCurrentPhase(phases, debug, landLabel, plantId = 0) {
...
function convertServerPhaseToClient(phases, serverPhaseInfo, plantId) {
    ...
    const isMature = serverPhase !== PlantPhase.DEAD && (serverPhase === PlantPhase.MATURE
        || serverPhase === MATURE_PHASE_RECORD_ID
        || phaseRecordId === MATURE_PHASE_RECORD_ID
        || (isFinalConfiguredPhase && (serverPhase === PlantPhase.GERMINATION
...
function hasRemainingSeasons(plant) {
    if (!plant)
        return false;
    const plantId = toNum(plant.id);
