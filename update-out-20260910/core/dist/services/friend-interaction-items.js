...
function buildInteractionInventory(predicate, targetKind = 'land', bagItems, baseContext) {
 ...
 */
async function runInteractionBatch(itemId, info, stacks, hostGid, landsInput, landIds, friendMode = true, propagateErrors = false) {
...
async function performFriendInteractionItemBatch(friendGidInput, itemIdInput, landIdsInput, propagateErrors = false) {
...
async function performFriendFarmInteractionItem(friendGidInput, itemIdInput, propagateErrors = false) {
    const friendGid = positiveDecimal(friendGidInput, 'INVALID_FRIEND_INTERACTION_GID', 'friendGid');
    const friendGidNumber = safePositiveNumber(friendGid, 'INVALID_FRIEND_INTERACTION_GID', 'friendGid');
    const itemId = safePositiveNumber(itemIdInput, 'INVALID_FRIEND_INTERACTION_ITEM_ID', 'itemId');
    const info = getItemById(itemId);
...
async function performSelfInteractionItemBatch(itemIdInput, landIdsInput, propagateErrors = false) {
...
function useFriendInteractionItemBatch(friendGidInput, itemIdInput, landIdsInput, propagateErrors = false) {
    return serializeMutation(() => performFriendInteractionItemBatch(friendGidInput, itemIdInput, landIdsInput, propagateErrors));
}
function useFriendFarmInteractionItem(friendGidInput, itemIdInput, propagateErrors = false) {
    return serializeMutation(() => performFriendFarmInteractionItem(friendGidInput, itemIdInput, propagateErrors));
}
function useSelfInteractionItemBatch(itemIdInput, landIdsInput, propagateErrors = false) {
