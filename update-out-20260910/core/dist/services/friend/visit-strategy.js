...
function getHelpSnapshotKey(lands) {
    return (Array.isArray(lands) ? lands : []).map((land) => {
        ...
        const phase = plant && Array.isArray(plant.phases) ? getCurrentPhase(plant.phases, false, '', toNum(plant && plant.id)) : null;
...
function analyzeFriendLands(lands, myGid, friendName = '', options = {}) {
    ...
    for (const land of lands) {
        ...
        const currentPhase = getCurrentPhase(plant.phases, false, `[${friendName}]土地#${id}`, toNum(plant.id));
        if (!currentPhase) {
            continue;
        }
...
async function getFriendsList(forceSync = false, priority = 'normal', propagateErrors = false) {
    try {
        // 检查缓存
        const now = Date.now();
 ...
 */
async function getFriendLandsDetail(friendGid, propagateErrors = false) {
    let entered = false;
    try {
...
// ============ 批量操作与面板操作 ============
async function runBatchWithFallback(ids, batchFn, singleFn, propagateErrors = false) {
...
async function runFarmingWithFallback(hostGid, ids, stopWhenExpLimit = false, snapshotKey = '', propagateErrors = false) {
 ...
 */
async function doFriendOperation(friendGid, opType, propagateErrors = false) {
    const gid = toNum(friendGid);
    if (!gid)
    ...
    try {
        ...
        if (opType === 'steal') {
            ...
            count = await runBatchWithFallback(target, (ids) => stealHarvest(gid, ids), (ids) => stealHarvest(gid, ids), propagateErrors);
            if (count > 0) {
        ...
        if (opType === 'farming' || opType === 'water' || opType === 'weed' || opType === 'bug') {
            ...
            const outcome = await runFarmingWithFallback(gid, landIds, false, getHelpSnapshotKey(lands), propagateErrors);
        ...
        if (opType === 'bad') {
            ...
            if (status.canPutWeed.length) {
                const weedRet = await putWeedsDetailed(gid, status.canPutWeed, propagateErrors);
            ...
            if (!schedulerRef().isBadOperationLimitReached() && status.canPutBug.length) {
                const bugRet = await putInsectsDetailed(gid, status.canPutBug, propagateErrors);
                bugCount = bugRet.ok;
