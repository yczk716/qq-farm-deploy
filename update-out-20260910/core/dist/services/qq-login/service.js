...
exports.QQ_MINIAPP_APP_ID = void 0;
exports.cancelLoginTask = cancelLoginTask;
exports.createLoginTask = createLoginTask;
exports.getMiniappCode = getMiniappCode;
exports.queryLoginStatus = queryLoginStatus;
const axios = require('axios').default;
const store = require('../../models/store');
const QQ_MINIAPP_APP_ID = '1112386029';
exports.QQ_MINIAPP_APP_ID = QQ_MINIAPP_APP_ID;
...
function napCatErrorMessage(data) {
...
async function requestNapCat(path, body, requireOk = true) {
...
async function queryLoginStatus(taskId) {
...
async function getMiniappCode(taskId) {
    const id = String(taskId || '').trim();
    if (!id)
        throw new Error('登录任务 ID 不能为空');
...
async function cancelLoginTask(taskId) {
    const id = String(taskId || '').trim();
    if (!id)
        throw new Error('登录任务 ID 不能为空');
